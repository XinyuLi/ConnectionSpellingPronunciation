...
def getProbSP():
    ...
    tranT = codecs.open(tranP2S,'r','utf8')
    for line in tranT:
        w = line.split(',')
        tran[w[0]] = float(w[1])
    emi = defaultdict(float)
    emiT = codecs.open(emiP2S,'r','utf8')
    for line in emiT:
        w = line.split(',')
        emi[w[0]] = float(w[1])
...
def parsing(obs):
    ...
    boLi = list()
    while i < l:
        if i < l-2 and obs[i:i+3] in cS:
            boLi.append(obs[i:i+3])
            i += 3
            continue
        if i < l-1 and obs[i:i+2] in cS:
            boLi.append(obs[i:i+2])
            i += 2
            continue
        boLi.append(obs[i])
        i += 1
...
def Viterbi(ClaList,transDict,emiDict,observ): # run Viterbi algorithm
    ...
    for i in range(1,c):
        ...
        for j in range(0,r):
            mtrx[j][i] = MIN_F
            for k in range(0,r):
                #log the result to avoid being zero
...
if __name__=='__main__':   #main function
    ...
    while 1:
        ...
        obs = parsing(word)
        [m,V] = Viterbi(cP,tra,emi,obs)
        print ("The pronunciation is:")
        print (V)
